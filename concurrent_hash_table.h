...
// Return whether the key was found.
template <typename Key, typename Mapped, typename Hash, typename Equal>
bool ConcurrentHashTable<Key, Mapped, Hash, Equal>::lookup(Mapped& destination, const Key& key) const {
...
template <typename Key, typename Mapped, typename Hash, typename Equal>
const Mapped *ConcurrentHashTable<Key, Mapped, Hash, Equal>::lookup(const Key& key) const {
    const std::size_t hash = Hash{}(key);
    const Shard& shard = shards[hash % shards.size()];
    std::shared_lock<std::shared_mutex> lock{shard.mutex};
    return shard.lookup(hash, key);
...
template <typename Key, typename Mapped, typename Hash, typename Equal>
bool ConcurrentHashTable<Key, Mapped, Hash, Equal>::insert(Key key, Mapped value) {
    const std::size_t hash = Hash{}(key);
    Shard& shard = shards[hash % shards.size()];
...
template <typename Key, typename Mapped, typename Hash, typename Equal>
Mapped *ConcurrentHashTable<Key, Mapped, Hash, Equal>::Shard::lookup(std::size_t hash, const Key& key) {
    ...
    auto found = std::find_if(bucket.begin(), bucket.end(), [&](const auto& entry) {
...
template <typename Key, typename Mapped, typename Hash, typename Equal>
const Mapped *ConcurrentHashTable<Key, Mapped, Hash, Equal>::Shard::lookup(std::size_t hash, const Key& key) const {
    // Note: Our caller is holding a reader lock on `mutex`.
...
bool ConcurrentHashTable<Key, Mapped, Hash, Equal>::Shard::insert(std::size_t hash, Key key, Mapped value) {
    ...
    if (buckets.empty() || double(num_elements + 1) / buckets.size() > max_load_factor) {
        ...
        const double min_growth_factor = (num_elements + 1) / (buckets.size() * max_load_factor);
        const double growth_factor = std::max(min_growth_factor, desired_growth_factor);
        ...
        decltype(buckets) new_buckets(buckets.empty() ? min_buckets : growth_factor * buckets.size());
